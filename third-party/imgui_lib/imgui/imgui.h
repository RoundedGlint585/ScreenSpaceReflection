...
// See imgui_lib.cpp file for documentation.
// Call and read ImGui::ShowDemoWindow() in imgui_demo.cpp for demo code.
// Newcomers, read 'Programmer guide' in imgui_lib.cpp for notes on how to setup Dear ImGui in your codebase.
...
// IMGUI_API is used for core imgui_lib functions, IMGUI_IMPL_API is used for the default bindings files (imgui_impl_xxx.h)
// Using dear imgui_lib via a shared library is not recommended, because of function call overhead and because we don't guarantee backward nor forward ABI compatibility.
#ifndef IMGUI_API
#define IMGUI_API
#endif
...
#ifndef ImTextureID
typedef void *ImTextureID;          // User data to identify a texture (this is whatever to you want it to be! read the FAQ about ImTextureID in imgui_lib.cpp)
#endif
...
// (Inside a namespace so you can add extra functions in your own separate file. Please don't modify imgui_lib.cpp/.h!)
//-----------------------------------------------------------------------------

namespace ImGui {
    // Context creation and access
    // Each context create its own ImFontAtlas by default. You may instance one yourself and pass it to CreateContext() to share a font atlas between imgui_lib contexts.
    ...
    IMGUI_API void StyleColorsClassic(ImGuiStyle *dst = NULL); // classic imgui_lib style
    ...
    IMGUI_API bool IsWindowHovered(
            ImGuiHoveredFlags flags = 0); // is current window hovered (and typically: not blocked by a popup/modal)? see flags for options. NB: If you are trying to check whether your mouse should be dispatched to imgui_lib or to your app, you should use the 'io.WantCaptureMouse' boolean for that! Please read the FAQ!
    ...
    IMGUI_API void PushItemWidth(
            float item_width);                                // set width of items for common large "item+label" widgets. >0.0f: width in pixels, <0.0f align xx pixels to the right of window (so -1.0f always align width to the right side). 0.0f = default to ~2/3 of windows width,
    ...
    // ID stack/scopes
    // - Read the FAQ for more details about how ID are handled in dear imgui_lib. If you are creating widgets in a loop you most
    ...
    // - Their visibility state (~bool) is held internally by imgui_lib instead of being held by the programmer as we are used to with regular Begin() calls.
    ...
    IMGUI_API bool BeginPopupContextVoid(const char *str_id = NULL,
                                         int mouse_button = 1);                                 // helper to open and begin popup when clicked in void (where there are no imgui_lib windows).
    ...
    // - The columns API is work-in-progress and rather lacking (columns are arguably the worst part of dear imgui_lib at the moment!)
    ...
    IMGUI_API bool SetDragDropPayload(const char *type, const void *data, size_t sz,
                                      ImGuiCond cond = 0);  // type is a user defined string of maximum 32 characters. Strings starting with '_' are reserved for dear imgui_lib internal types. Data is copied and held by imgui_lib.
    IMGUI_API void
    ...
    IMGUI_API double
    GetTime();                                                          // get global imgui_lib time. incremented by io.DeltaTime every frame.
    IMGUI_API int
    GetFrameCount();                                                    // get global imgui_lib frame count. incremented by 1 every frame.
    IMGUI_API ImDrawList *
    GetBackgroundDrawList();                                            // this draw list will be the first rendering one. Useful to quickly draw shapes/text behind dear imgui_lib contents.
    IMGUI_API ImDrawList *
    GetForegroundDrawList();                                            // this draw list will be the last rendered one. Useful to quickly draw shapes/text over dear imgui_lib contents.
    ...
    IMGUI_API bool IsKeyDown(
            int user_key_index);                                      // is key being held. == io.KeysDown[user_key_index]. note that imgui_lib doesn't know the semantic of each entry of io.KeysDown[]. Use your own indices/enums according to how your backend/engine stored them into io.KeysDown[]!
    ...
    // - The disk functions are automatically called if io.IniFilename != NULL (default is "imgui_lib.ini").
    ...
    // Memory Allocators
    // - All those functions are not reliant on the current context.
    // - If you reload the contents of imgui_lib.cpp at runtime, you may need to call SetCurrentContext() + SetAllocatorFunctions() again because we use global storage for those.
...
// Note: if you are trying to check whether your mouse should be dispatched to imgui_lib or to your app, you should use the 'io.WantCaptureMouse' boolean for that. Please read the FAQ!
...
enum ImGuiDragDropFlags_ {
    ...
    ImGuiDragDropFlags_SourceAllowNullID = 1
            << 3,   // Allow items such as Text(), Image() that have no unique identifier to be used as drag source, by manufacturing a temporary identifier based on their window-relative position. This is extremely unusual within the dear imgui_lib ecosystem and so we made it explicit.
    ImGuiDragDropFlags_SourceExtern = 1
            << 4,   // External source (from outside of dear imgui_lib), won't attempt to read current item/window info. Will always return true. Only one Extern source can be active simultaneously.
...
// Read instructions in imgui_lib.cpp for more details. Download PNG/PSD at http://goo.gl/9LgVZW.
enum ImGuiNavInput_ {
    // Gamepad Mapping
...
enum ImGuiConfigFlags_ {
    ...
    ImGuiConfigFlags_NavEnableGamepad = 1
            << 1,   // Master gamepad navigation enable flag. This is mostly to instruct your imgui_lib back-end to fill io.NavInputs[]. Back-end also needs to set ImGuiBackendFlags_HasGamepad.
    ...
    ImGuiConfigFlags_NoMouse = 1
            << 4,   // Instruct imgui_lib to clear mouse position/buttons in NewFrame(). This allows ignoring the mouse information set by the back-end.
    ImGuiConfigFlags_NoMouseCursorChange = 1
            << 5,   // Instruct back-end to not alter mouse cursor shape and visibility. Use if the back-end cursor changes are interfering with yours and you don't want to use SetMouseCursor() to change mouse cursor. You may want to honor requests from imgui_lib by reading GetMouseCursor() yourself instead.
...
// You generally do NOT need to care or use this ever. But we need to make it available in imgui_lib.h because some of our data structures are relying on it.
// Important: clear() frees memory, resize(0) keep the allocated buffer. We use resize(0) a lot to intentionally recycle allocated buffers across frames and amortize our costs.
// Important: our implementation does NOT call C++ constructors/destructors, we treat everything as raw data! This is intentional but be extra mindful of that,
// do NOT use this class as a std::vector replacement in your own code! Many of the structures used by dear imgui_lib can be safely initialized by a zero-memset.
//-----------------------------------------------------------------------------
...
struct ImGuiIO {
    ...
    const char *IniFilename;                    // = "imgui_lib.ini"    // Path to .ini file. Set NULL to disable automatic .ini loading/saving, if e.g. you want to manually load/save from memory.
    ...
    bool WantCaptureMouse;               // When io.WantCaptureMouse is true, imgui_lib will use the mouse inputs, do not dispatch them to your main game/application (in both cases, always pass on mouse inputs to imgui_lib). (e.g. unclicked mouse is hovering over an imgui_lib window, widget is active, mouse was clicked over an imgui_lib window, etc.).
    bool WantCaptureKeyboard;            // When io.WantCaptureKeyboard is true, imgui_lib will use the keyboard inputs, do not dispatch them to your main game/application (in both cases, always pass keyboard inputs to imgui_lib). (e.g. InputText active, or an imgui_lib window is focused and navigation is enabled, etc.).
    ...
    int MetricsActiveAllocations;       // Number of active allocations, updated by MemAlloc/MemFree based on current context. May be off if you have multiple imgui_lib contexts.
    ...
    bool MouseDownOwned[5];              // Track if button was clicked inside a dear imgui_lib window. We don't request mouse capture from the application if click started outside ImGui bounds.
...
struct ImGuiPayload {
    // Members
    void *Data;               // Data (copied and owned by dear imgui_lib)
...
//-----------------------------------------------------------------------------
// Obsolete functions (Will be removed! Read 'API BREAKING CHANGES' section in imgui_lib.cpp for details)
// Please keep your copy of dear imgui_lib up to date! Occasionally set '#define IMGUI_DISABLE_OBSOLETE_FUNCTIONS' in imconfig.h to stay ahead.
//-----------------------------------------------------------------------------
...
// Each dear imgui_lib window contains its own ImDrawList. You can use ImGui::GetWindowDrawList() to
...
struct ImDrawList {
    ...
    AddImageQuad(ImTextureID user_texture_id, const ImVec2 &a, const ImVec2 &b, const ImVec2 &c, const ImVec2 &d,
                 ...
                 const ImVec2 &uv_c = ImVec2(1, 1), const ImVec2 &uv_d = ImVec2(0, 1), ImU32 col = IM_COL32_WHITE);
...
// Include imgui_user.h at the end of imgui_lib.h (convenient for user to only explicitly include vanilla imgui_lib.h)
