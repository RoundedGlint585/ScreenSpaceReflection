...
// - We never use any of the helpers/facilities used internally by dear imgui_lib, unless it has been exposed in the public API (imgui_lib.h).
// - We never use maths operators on ImVec2/ImVec4. For other imgui_lib sources files, they are provided by imgui_internal.h w/ IMGUI_DEFINE_MATH_OPERATORS,
...
void ImGui::ShowDemoWindow(bool *p_open) {
    IM_ASSERT(ImGui::GetCurrentContext() != NULL &&
              "Missing dear imgui_lib context. Refer to examples app!"); // Exceptionally add an extra assert here for people confused with initial dear imgui_lib setup
    ...
    ImGui::Text("dear imgui_lib says hello. (%s)", IMGUI_VERSION);
    ImGui::Spacing();

    if (ImGui::CollapsingHeader("Help")) {
        ImGui::Text("PROGRAMMER GUIDE:");
        ImGui::BulletText("Please see the ShowDemoWindow() code in imgui_demo.cpp. <- you are here!");
        ImGui::BulletText("Please see the comments in imgui_lib.cpp.");
...
static void ShowDemoWindowWidgets() {
    ...
    if (ImGui::TreeNode("Basic")) {
...

        {
            ...
            const char *current_element_name = (current_element >= 0 && current_element < Element_COUNT)
                                               ? element_names[current_element] : "Unknown";
        ...
        if (ImGui::TreeNode("Filtered Text Input")) {
            ...
            struct TextFilters {
                static int FilterImGuiLetters(ImGuiInputTextCallbackData *data) {
                    if (data->EventChar < 256 && strchr("imgui_lib", (char) data->EventChar))return 0;
                    return 1;
                }
            };
            static char buf6[64] = "";
            ImGui::InputText("\"imgui_lib\" letters", buf6, 64, ImGuiInputTextFlags_CallbackCharFilter,
    ...
    if (ImGui::TreeNode("Data Types")) {
        ...
        ImGui::SameLine();
        HelpMarker(
                "As with every widgets in dear imgui_lib, we never modify values unless there is a user interaction.\nYou can override the clamping limits by using CTRL+Click to input a value.");
...
static void ShowDemoWindowPopups() {
    ...
    if (ImGui::TreeNode("Modals")) {
        ...
        if (ImGui::BeginPopupModal("Stacked 1", NULL, ImGuiWindowFlags_MenuBar)) {
            ...
            // Note that the visibility state of popups is owned by imgui_lib, so the input value of the bool actually doesn't matter here.
...
void ImGui::ShowAboutWindow(bool *p_open) {
    ...
    ImGui::Separator();
    ImGui::Text("By Omar Cornut and all dear imgui_lib contributors.");
...
static void ShowExampleAppCustomRendering(bool *p_open) {
    ...
    // ImGui defines overloaded operators but they are internal to imgui_lib.cpp and not exposed outside (to avoid messing with your types)
